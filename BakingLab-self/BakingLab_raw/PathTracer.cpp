                                       ...
                                       Float3& irradiance, Float3& sampleDir)
{
    ...
    const float invPDF = 2.0f * Pi * radius2;//radius2
    ...
    //sampleDir = lightPos - position;
    float sampleDirLen = Float3::Length(sampleDir);// light direction
    bool visible = false;
    ...
    float attenuation = areaNDotL * invRSqr;//areaNDotL
    if(attenuation && visible)
    {
        Float3 sampleIrradiance = Saturate(Float3::Dot(normal, sampleDir)) * lightColor * attenuation;//sample sunlight in curren hitpos
        Float3 sample = CalcLighting(normal, sampleIrradiance, sampleDir, diffuseAlbedo, position,
                                     cameraPos, roughness, includeSpecular, specAlbedo);//brdf
                             ...
                             float u1, float u2, Float3& irradiance)
{
    ...
    const float invPDF = 2.0f * Pi * radius2;//140
    Float3 result = 0.0f;
    sampleDir = sunPos - position;
    float sampleDirLen = Float3::Length(sampleDir);
    sampleDir = Float3::Normalize(sampleDir);
    bool visible = false;
    bool hidden = Occluded(scene, position, sampleDir, 0.1f, sampleDirLen);
    visible = !hidden;
    float invRSqr = 1.0f / (sampleDirLen * sampleDirLen);
    float attenuation = 0.6f * invRSqr;//areaNDotL
    if (attenuation && visible)
    {
        Float3 sampleIrradiance = Saturate(Float3::Dot(normal, sampleDir)) * sunLuminance * attenuation;
        Float3 sample = InvPi * sampleIrradiance;
        result = sample * 140;//invPDF
    }
    return result;
...
Float3 PathTrace(const PathTracerParams& params, Random& randomGenerator, float& illuminance, bool& hitSky)
{
    ...
    for(int64 pathLength = 1; pathLength <= maxPathLength || maxPathLength == -1; ++pathLength)//使用轮盘赌来决定反弹次数, 不受for循环影响
    {
        ...
        if(pathLength >= rouletteDepth && rouletteDepth != -1)//步数过长时, 进行轮盘赌判断
        {
        ...
        float sceneDistance = ray.Hit() ? ray.tfar : FLT_MAX;//场景距离
        ...
        else if(sceneDistance < FLT_MAX)//hit scene, then checke each triangle
        {
            ...
            //{
                ...
                if((AppSettings::EnableDirectLighting/*true when bake*/ || pathLength > 1) && AppSettings::EnableSun)
                {
 
                    Float2 sunSample = randomGenerator.RandomFloat2();
                    ...
                    {
                        directLighting += sunDirectLighting;//important
                    }
                }

                radiance += directLighting * throughput;
                irradiance += directIrradiance * irrThroughput;
            //}//if only bouce once (path tracing no indirectilight, lightmap one bounce)


            if(AppSettings::EnableIndirectLighting || params.ViewIndirectSpecular)//根据hitpoint的brdf再选择反射ray方向
            {
